...
interface Message {
  sender: 'user' | 'sorren';
  text: string;
  fontSize?: string; // Add fontSize property to Message
}
...
function SorrenChatbot() {
  ...
  const getDynamicFontSize = (text: string) => {
    const baseSize = 2; // base font size in em
    if (text.length > 150) return `${baseSize - 0.5}em`;
    if (text.length > 100) return `${baseSize - 0.3}em`;
    return `${baseSize}em`;
  };
  ...
  const simulateTyping = (text: string) => {
    ...
    const typingInterval = setInterval(() => {
      if (index < text.length) {
        ...
        const newMessage: Message = { sender: 'sorren', text, fontSize: getDynamicFontSize(text) }; // Set fontSize once and cast as Message
  ...
  // Trigger the initial message with typing effect on component load
  useEffect(() => {
